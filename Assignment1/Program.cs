    ...
    class Program
    {
        ...
        */
        ...
        private static bool HalvesAreAlike(string s)
        {
            try
            {
                //Finding the middle of the string to check both halves
                ...
                for (int i = 0, j = middle; i < middle; i++, j++)
                {
                    if (sVowels.IndexOf(s[i]) >= 0)
                        //counter increments by 1 if a vowel is found in first half
                        count++;
                    if (sVowels.IndexOf(s[j]) >= 0)
                        //counter decremenst by 1 if a vowel is found in first half
                        count--;
                }
                //if count==0 is ture then both halves are equal
                return count == 0;
            }
            catch (Exception)
            {
        ...
        private static bool CheckIfPangram(string s)
        {
            try
            {
                ...
                //boolean array to store whether all characters are present
                bool[] check = new bool[26];
                bool isPan = true;
                //to store the index of each character of the string    
                int index = 0;

                for (int i = 0; i < s.Length; i++)
                {
                    //parsing through all uppercase characters and storing their index
                    if ('A' <= s[i] && s[i] <= 'Z')
                        index = s[i] - 'A';
                    //parsing through all lower case characters and storing their index
                    else if ('a' <= s[i] && s[i] <= 'z')
                        index = s[i] - 'a';
                ...
                //if all the characters are present then the string is pangram else we return false 
                for (int i = 0; i <= 25; i++)
                    if (check[i] == false)
                        isPan = false;
        ...
        private static int MaximumWealth(int[,] accounts)
        {
            try
            {
                //initialised the maximum wealth =0
                int iMaxWealth = 0;
                for (int i = 0; i < accounts.GetLength(0); i++)
                {
                    int total = 0;
                    for (int j = 0; j < accounts.GetLength(1); j++)
                        //computing the total deposited amount of each users
                        total = total + accounts[i, j];
                    //comparing the maximum wealth of all users
                    //(the first users total is compared with the initialised value of maximum wealth i.e 0)
        ...
        private static int NumJewelsInStones(string jewels, string stones)
        {
            try
            {
                //set the counter to count stones that are jewels
                int count = 0;
                for (int i = 0; i < stones.Length; i++)
                {
                    for (int j = 0; j < jewels.Length; j++)
                    {
                        //comparing if stones are jewels and incrementing if yes
        ...
        private static string RestoreString(string s, int[] indices)
        {
            try
            {
                //array to restore the sting after swapping
                char[] restore = new char[s.Length];

                for (int i = 0; i < s.Length; i++)
                    //computing the characters at their right index
                    restore[indices[i]] = s[i];
                //storing the reverted string
        ...
        private static int[] CreateTargetArray(int[] nums, int[] index)
        {
            try
            {
                ...
                //creating a list to make it easy to insert numbers at their appropriate index
                List<int> nTarget = new List<int>(index.Length);
                for (int i = 0; i < index.Length; i++)
                    //inserting numbers at the right index
                    nTarget.Insert(index[i], nums[i]);
                //converting the list to array before returning
